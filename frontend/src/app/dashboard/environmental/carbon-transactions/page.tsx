...
export default function CarbonTransactionsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      fetch('/api/carbon-transactions').then(r => r.json()).then(setTransactions);
      setForm({ departmentId: '', sourceType: 'Manufacturing', quantity: '', emissionFactorId: '' });
    }
  };
    ...
    <div>
      ...
      <div className="card card-green" style={{ marginBottom: '28px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr auto', gap: '14px', alignItems: 'end' }}>
          <div className="form-group" style={{ margin: 0 }}><label className="form-label">Department</label><select className="form-select" value={form.departmentId} onChange={e => setForm({...form, departmentId: e.target.value})} required><option value="">Select...</option>{depts.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
